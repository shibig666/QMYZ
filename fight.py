...
def write_csv(
    ...
    with open(csv_file_path, "a", encoding="utf-8", newline="") as csvfile:
        ...
        print(f"Adding a new topic")
    tiku = loadCSV(csv_file_path)


# 请求相关参数
cookies = {
    "JSESSIONID": JSESSIONID if JSESSIONID != "" else get_cookie_from_url(url),
}
...
headers = {
    ...
    "Pragma": "no-cache",
    "Referer": "http://112.5.88.114:31101/yiban-web/stu/toCombatLoading.jhtml?fightType=1&courseId="
    + courseId,
...
def main():
    ...
    start_sub = json.loads(
        re.findall(r'{"courseId":.*}', start.text)[0]
    )  # 获取html中的题目
    flag = False
    ans = "A"
    for l in tiku:
        if str(start_sub["subjectId"]) == l[1] and str(start_sub["courseId"]) == l[0]:
            print("找到题目")
            ans = l[-1]
            flag = True
            ansFlag = False
    ...
    for i in range(5):
        ...
        for l in tiku:
            if (
                str(nextSub.json()["data"]["subject"]["id"]) == l[1]
                and str(nextSub.json()["data"]["subject"]["courseId"]) == l[0]
            ):
                print("答对了")
                ans = l[-1]
                flag = True
